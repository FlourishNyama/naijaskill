...
import { Search, MapPin, Star, Filter, ShieldCheck, SlidersHorizontal, Loader2 } from 'lucide-react';
...
export default function BrowsePage() {
  ...
  useEffect(() => {
    const fetchArtisans = async () => {
      ...
      // Get profiles where role is 'artisan'
      let query = supabase
        .from('profiles')
        .select('*')
        .eq('role', 'artisan');
                  ...
                  <Link href={`/profile/${artisan.id}`} key={artisan.id} className="group bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden hover:shadow-lg hover:border-green-200 dark:hover:border-green-700 transition">
                    ...
                    <div className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="font-bold text-gray-900 dark:text-white line-clamp-1">{artisan.full_name}</h3>
                          <p className="text-xs text-green-600 dark:text-green-400 font-bold uppercase">{artisan.job_title || "Artisan"}</p>
                        </div>
