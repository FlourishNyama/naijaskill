...
import { 
  LayoutDashboard, Briefcase, MessageSquare, Wallet, Settings, LogOut, Clock, CheckCircle, ShieldCheck, Plus, Repeat, Loader2, ChevronDown, ArrowUpRight
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 flex pb-20 md:pb-0 transition-colors duration-300"> 
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8">
        ...
        {/* Header - MOBILE PERFECTED */}
        <div className="flex justify-between items-center mb-6 md:mb-8 mt-2 md:mt-0 relative z-20">
          <div>
            {/* MOBILE LOGO (Click to go Home) */}
            <Link href="/" className="md:hidden text-xl font-bold text-green-900 dark:text-white tracking-tight">
              Naija<span className="text-green-600 dark:text-green-400">Skill</span>
            </Link>
            
            {/* DESKTOP WELCOME */}
            <h1 className="hidden md:block text-2xl font-bold text-gray-900 dark:text-white">Dashboard</h1>
            <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">
              Hello, <span className="font-bold text-green-600">{user?.user_metadata?.full_name || "Client"}</span>
            </p>
          </div>
          
          <div className="flex items-center gap-3">
             <Link href="/artisan-dashboard" className="md:hidden p-2 bg-green-50 dark:bg-slate-800 text-green-600 dark:text-green-400 rounded-full border border-green-200 dark:border-slate-700"><Repeat className="w-5 h-5" /></Link>
             ...
             <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-2 focus:outline-none">
                <div className="h-10 w-10 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center text-green-700 dark:text-green-400 font-bold border-2 border-white dark:border-slate-800 shadow-sm overflow-hidden">
                    {user?.user_metadata?.avatar_url ? <Image src={user.user_metadata.avatar_url} alt="Profile" fill className="object-cover" /> : user?.user_metadata?.full_name?.substring(0, 2).toUpperCase() || "CN"}
                </div>
             ...
             {isProfileOpen && (
               <>
                 ...
                 <div className="absolute right-0 mt-12 md:mt-2 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-gray-100 dark:border-gray-800 z-20 overflow-hidden">
          ...
          <div className="p-4 md:p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-start">
            <div className="flex items-center space-x-3 md:space-x-4">
              <div className="relative w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden bg-gray-100 dark:bg-slate-800">
                <Image src="https://images.unsplash.com/photo-1581578731117-104f2a8d23e9?q=80&w=2940&auto=format&fit=crop" alt="Emmanuel" fill className="object-cover" />
              </div>
              <div>
            ...
            <span className="bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 px-2 py-1 md:px-3 rounded-full text-[10px] md:text-xs font-bold flex items-center"><Clock className="w-3 h-3 mr-1" /> In Progress</span>
          </div>
          <div className="bg-gray-50 dark:bg-slate-800/50 p-4 flex flex-col md:flex-row justify-between items-center gap-3">
             <button className="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white font-medium text-sm w-full md:w-auto text-center">View Contract</button>
             <button className="bg-green-600 hover:bg-green-700 dark:bg-green-600 dark:hover:bg-green-500 text-white px-6 py-3 md:py-2 rounded-lg font-bold text-sm shadow-sm flex items-center justify-center w-full md:w-auto"><CheckCircle className="w-4 h-4 mr-2" /> Mark Complete</button>
          </div>
        </div>
